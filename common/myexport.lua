...
function myexp.export_set()
    ...
    local set_slot = function(slots)
        local slot = ''
        for s in pairs(slots) do
            if s == 0 then slot = 'main'
            elseif s== 1 then 
                if slot ~= 'main' then 
                    slot = 'sub'
                end
            elseif s== 2 then slot = 'range'
            elseif s== 3 then slot = 'ammo'
            elseif s== 4 then slot = 'head'
            elseif s== 5 then slot = 'body'
            elseif s== 6 then slot = 'hands'
            elseif s== 7 then slot = 'legs'
            elseif s== 8 then slot = 'feet'
            elseif s== 9 then slot = 'neck'
            elseif s== 10 then slot = 'waist'
            elseif s== 11 then slot = 'ear'
            elseif s== 12 then slot = 'ear'
            elseif s== 13 then slot = 'ring'
            elseif s== 14 then slot = 'ring'
            elseif s== 15 then slot = 'back'
            end
        end
        return slot
    end
        ...
        for n,m in ipairs(wardrobe) do
            ...
            if m.id ~= 0 then
                ward_all_item[id]:append({
                    ...
                    count=m.count,
                    slot=set_slot(gearswap.res.items[m.id].slots)
                })
            else
    ...
    for _,id in ipairs(ward_ids) do
        f_ward_all:write(gearswap.res.bags[id].command..' = '..(ward_all_item[id]:tovstring({'augments','log_name','name','id','count','slot'}))..',\n')
    end
    f_ward_all:write('}\n')
    f_ward_all:close()
